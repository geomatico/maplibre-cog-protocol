...
function isValidColorSchemeName(name) {
    return colorSchemeNames.includes(name);
}
const hexToIntColorRegexp = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i;
const hexToIntColor = (color) => {
...
const intervals = (min, max, n) => Array.from({ length: n }, (_x, i) => min + i * (max - min) / (n - 1));
const thresholds = (min, max, n) => Array.from({ length: n - 1 }, (_x, i) => min + (i + 1) * (max - min) / n);
const colorScale = ({ colorScheme, customColors, min, max, isReverse = false, isContinuous = false }) => {
    let colors;
    if (colorScheme) {
