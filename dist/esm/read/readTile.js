...
const arrayTypeFor = (image) => {
    ...
    if (bitsPerSample > 8 && !image.littleEndian)
        return undefined; // host order is little endian
 ...
 */
const bindDecoder = async (image, compression, predictor, pool) => {
    const extra = compression === JPEG
 ...
 */
const sourceIndexes = (windowStart, windowEnd, size) => {
 ...
 */
export const readTileFast = async (image, { window: [left, top, right, bottom], width, height, fillValue, pool }) => {
    if (image.planarConfiguration !== 1)
        return undefined; // one block per band, not per pixel
    ...
    const isWholeBlock = width === blockWidth &&
        height === blockHeight &&
    ...
    if (isWholeBlock) {
        ...
        if (values.length === blockWidth * blockHeight * samples)
            return values;
    }
    ...
    for (let x = 0; x < width; x++) {
        ...
        const isContiguous = previous !== undefined &&
        ...
        if (isContiguous)
            ...
            runs.push({ block, source: columnOffsets[x], column: x, length: 1 });
    }
    ...
    for (const row of rows) {
        if (row >= 0 && row < imageHeight)
            blockRows.add(Math.floor(row / blockHeight));
    }
    const decoder = await bindDecoder(image, compression, predictor, pool);
    const blockLength = blockWidth * blockHeight * samples;
    const blocks = new Map();
    const decoded = await Promise.all([...blockRows].flatMap((blockRow) => [...blockColumns].map(async (blockColumn) => {
    ...
    if (decoded.some((length) => length !== blockLength))
        return undefined;
    const raster = new ArrayType(width * height * samples);
    if (fillValue)
        raster.fill(fillValue);
    for (let y = 0; y < height; y++) {
        ...
        for (const run of runs) {
            ...
            if (block === undefined || from + values > block.length)
                continue; // sparse or short block
            ...
            if (values > 4) {
                ...
                for (let i = 0; i < values; i++)
                    raster[destination + i] = block[from + i];
            }
        }
    }
    return raster;
