...
const coveredRange = (
  ...
  const clamp = (value: number) => Math.min(Math.max(value, 0), tileSize);
  ...
  if (isLastWindowPixelCovered) return [last > first ? first : clampedFrom, tileSize];
 ...
 */
export const pixelWindowToTileCoverage = (
